...
Shader "NextGenSprites/Standard Shadowy/Multi" {
	Properties {
		//Sprite Props
	...
	SubShader {
		...
		Pass {
			...
			#pragma shader_feature TRANSMISSION_ON
			...
			VertexOutput vert (VertexInput v) {
				...
				half3 diffuse = 1 * _LightColor0.rgb * NdotL;
				half3 surface = diffuse + UNITY_LIGHTMODEL_AMBIENT.rgb;
			...
			float4 frag(VertexOutput i) : COLOR{
				//Define empty variables to reduce conditional logic
				half3 emissive = 0;
				half3 specular = 0;
				half3 backFace = 0;
				...
				//Texture to RGBA
					...
					//Result
					half3 diffuseColor = canvas * i.vertexColor.rgb * LIGHT_ATTENUATION(i);
				#else
					//Just tint
					half4 _MainTex_var = tex2D(_MainTex, TRANSFORM_TEX(i.uv0, _MainTex));
					...
					half3 diffuseColor = _MainTex_var.rgb * i.vertexColor.rgb * LIGHT_ATTENUATION(i);
				#endif
				...
				//Curvature
					...
					half3x3 tangentTransform = half3x3(i.tangentDir, i.bitangentDir, normalDirection);
					...
					specular = (1 * _LightColor0.rgb) * pow(max(0, dot(halfDirection, normalDirection)), specPow) * _SpecColor.rgb;
				#endif
				...
				//Transmission
					...
					half3 backLight = (-NdotL + 1) * lerp(diffuseColor, _TransmissionTex_var.rgb, density);
					backFace = backLight;
				#endif
				...
				//Final Color
				half3 finalColor = diffuseColor + (backFace * (1 * _LightColor0.rgb)) + specular + emissive;
		...
		Pass {
			...
			VertexOutput vert (VertexInput v) {
				...
				o.vertexColor = v.vertexColor * float4((_Color.rgb + UNITY_LIGHTMODEL_AMBIENT.rgb), _Color.a);
				o.normalDir = UnityObjectToWorldNormal(v.normal);
			...
			float4 frag(VertexOutput i): COLOR {
				...
				//Get Canera View Direction
				#if CURVATURE_ON || DOUBLESIDED_ON || TRANSMISSION_ON || REFLECTION_ON
					half3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
				#endif
				...
				half3 attenColor = LIGHT_ATTENUATION(i) * _LightColor0.rgb;
				...
				//Curvature
					...
					normalDirection = normalize(mul( normalLocal, tangentTransform ));
					half specPow = exp2( 0.5 * 10.0 + 1.0);
					half3 specularColor = _SpecColor.rgb;
					half3 halfDirection = normalize(viewDirection + lightDirection);
					specular = (attenColor * _LightColor0.rgb) * pow(max(0, dot(halfDirection, normalDirection)), specPow) * specularColor;
				#endif
				...
				//Transmission
					...
					half3 forwardLight = max(0.0, NdotL);
					half4 _TransmissionTex_var = tex2D(_TransmissionTex, TRANSFORM_TEX(i.uv0, _TransmissionTex));
					half density = (_TransmissionDensity * -1) + 1;
					half3 backLight = max(0.0, -NdotL ) * lerp(diffuseColor, _TransmissionTex_var.rgb, density);
					half3 directDiffuse = (forwardLight + backLight) * attenColor;
				#else
					half NdotL = dot(normalDirection, lightDirection);
				...
				return fixed4(finalColor * (_MainTex_var.a * i.vertexColor.a), 0);
			}
			ENDCG
		}
		Pass {
