...
Shader "NextGenSprites/Standard Shadowy/Unlit" {
	Properties {
		//Sprite Props
		...
		Pass {
			...
			float4 frag(VertexOutput i) : COLOR {
				...
				//Texture to RGBA
					...
					//Result
					half3 emissive = canvas * LIGHT_ATTENUATION(i);
				#else
					//Just tint
					half4 _MainTex_var = tex2D(_MainTex, TRANSFORM_TEX(i.uv0, _MainTex));
					...
					half3 emissive = _MainTex_var.rgb * i.vertexColor.rgb * LIGHT_ATTENUATION(i);
				#endif
